...
interface CleanOptions {
     ...
     * 白名单字段（仅保留这些字段）
     */
    includeKeys?: Set<string | number> | (string | number)[] | Record<string, boolean> | ((key: string | number, value: any) => boolean);
    /**
     * 黑名单字段（排除这些字段）
     */
    excludeKeys?: Set<string | number> | (string | number)[] | Record<string, boolean> | ((key: string | number, value: any) => boolean);
    /**
     * 指定需要从某些对象字段中移除的子字段
     */
    stripKeysInObjects?: {
        [objectKey: string]: (string | ((key: string, value: any) => boolean))[] | ((key: string, value: any) => boolean);
    };
    /**
     * 清理空对象 {}
     */
     ...
     * 清理空白字符串 "  "
     */
    excludeWhitespaceStrings?: boolean;
    /**
     * 自定义字段过滤函数
     */
    customFilter?: (value: any, key?: string | number | symbol, parent?: any) => boolean;
    /**
     * 最大递归深度（防止栈溢出）
     */
    maxDepth?: number;
    /**
     * 是否允许顶层返回 `undefined`（默认：`false`）
     */
    allowRootIfEmpty?: boolean;
}
type isInvalidLiteral<T> = T extends null | undefined | "" ? true : false;
type DeepClean<T> = T extends object ? {
    [K in keyof T as isInvalidLiteral<T[K]> extends true ? never : K]: DeepClean<T[K]>;
} : T;
/**
 * packageSize.ts
 */
...
/**
 * 将传入的参数转换为一个 `Set` 类型的对象
 */
declare function toSet(value: any): Set<any> | undefined;
/**
 * 判断是否要清理某个字段
 */
declare function shouldCleanKey(key: string | number, value: any, options: CleanOptions): boolean;
/**
 * 检查值是否无效
 */
declare function isInvalidValue(value: any, options: CleanOptions): boolean;
/**
 * @description 从对象中移除无效值（如 `null`、`undefined`、空字符串、空数组、空对象）和不需要的字段
 * @param obj 需要清理的对象
 * @param options 可选属性
 * @returns 返回移除无效值后的新对象，不会修改原始对象
 * @see 详细文档 https://pure-admin-utils.netlify.app/utils/object/object#cleanObject
 */
declare function cleanObject<T>(obj: T, options?: CleanOptions): DeepClean<T> | undefined;
...
export { AmountOpt, AppendDataOpts, ArgsDraggable, ArgumentsType, Arrayable, AttrParams, Awaitable, BrowserType, CleanOptions, ColorOptions, ColorType, ComponentElRef, ComponentRef, ComputedRefValue, Constructor, DarkOptions, DataURL, DeepClean, DeepMerge, DeviceType, DivideOptions, EaseInOutQuadOptions, EchartGlobalProperties, EchartOptions, ElRef, ElementOf, ElementRef, EventParams, Fn, FormDataOptions, FunctionArgs, GlobalUtilsPropertiesApi, GradientOptions, HandleFileType, HideTextIndex, LoadOpts, LoadResult, MergeInsertions, MouseEvent, Nullable, OptionsParams, OptionsScript, OrderType, Performance, ProxyStorage, ReadonlyRecordable, Recordable, RefType, RefValue, SFCInstallWithContext, SFCWithInstall, ScrollOptions, SvgInfo, Target, Theme, TimeoutHandle, ToolTipType, UnionToIntersection, UseResizeObserverOptions, UtilsEChartsOption, addClass, addZero, addition, allowMouseEvent, appendFieldByUniqueId, arrayAllExist, arrayAllExistDeep, arrayAnyExist, arrayAnyExistDeep, attr, average, banMouseEvent, bd09togcj02, buildGUID, buildHierarchyTree, buildPrefixUUID, buildUUID, centsToDollars, cleanObject, clone, cloneDeep, convertImageToGray, convertPath, copyTextToClipboard, createFormData, createYear, currentDateOpt, currentDateType, darken, dataURLtoBlob, dateFormat, dateType, debounce, deepEqual, delObjectProperty, delay, deleteChildren, deviceDetection, divisionOperation, dollarsToCents, downloadByBase64, downloadByData, downloadByOnlineUrl, downloadByUrl, easeInOutQuad, entries, exceedMathMax, extractFields, extractPathList, formDataHander, formatBytes, gcj02tobd09, gcj02towgs84, getBrowserInfo, getClass, getCurrentDate, getCurrentWeek, getDecimalPlaces, getKeyList, getLocation, getNodeByUniqueId, getPackageSize, getPerformance, getQueryMap, getSvgInfo, getTime, gradientType, grayOpt, handleTree, hasCNChars, hasClass, hasOwnProp, hexToRgb, hideTextAtIndex, hmsType, intersection, is, isAllEmpty, isAlphabets, isArray, isBase64, isBoolean, isBrowser, isClient, isDate, isDef, isElement, isEmail, isEmpty, isEqual, isEqualArray, isEqualObject, isExistSpace, isFunction, isHex, isHtml, isIncludeAllChildren, isInvalidLiteral, isInvalidValue, isJSON, isLeapYear, isLowerCase, isNull, isNullAndUnDef, isNullOrUnDef, isNumber, isObject, isParams, isPhone, isPlainObject, isPostCode, isPromise, isQQ, isRegExp, isRgb, isRgba, isServer, isString, isUnDef, isUpperCase, isUrl, isWindow, lighten, mapsEqual, max, min, monthDays, multiplication, nameCamelize, nameHyphenate, numberToChinese, openLink, out_of_china, packageOpt, priceToThousands, priceUppercase, randomColor, randomDivide, randomGradient, removeAllSpace, removeBothSidesSpace, removeClass, removeLeftSpace, removeRightSpace, rgbToHex, setsEqual, shouldCleanKey, shuffleArray, splitNum, storageLocal, storageSession, subAfter, subBefore, subBetween, subBothSides, subTextAddEllipsis, subtraction, sum, swapOrder, throttle, toSet, toggleClass, urlToBase64, useAttrs, useCopyToClipboard, useDark, useDraggable, useDynamicComponent, useECharts, useGlobal, useLoader, useResizeObserver, useScrollTo, useWatermark, uuid, wgs84togcj02, withInstall, withInstallFunction, withNoopInstall };
